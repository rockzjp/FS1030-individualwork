...
class BlogAdminListContent extends React.Component {
 ...
 constructor(props) {
  ...
  update(param,event){
    console.log(param)
    console.log(event)
    window.location = '/blog_list_edit?id=' + param.id;
  }
  ...
  updateList()
  {
      ...
      $.post('http://localhost:3001/portfolio/search',{},function(res){
      _this.setState({
          rows: res.portfolioInfo
        });
      })
  }
      ...
      <Container maxWidth="lg" className={this.state.classes.blogContainer}>
             <Typography variant="h4" className={this.state.classes.blogTitle}>
              ...
              <Table>
                ...
                <a href="/blog_list_add">Add New</a>
      </Container>
    </div>
  );
  }
...
export default BlogAdminListContent;
